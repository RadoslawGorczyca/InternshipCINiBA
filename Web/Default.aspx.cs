        ...
        protected void BSend_Click(object sender, EventArgs e)
        {
            ...
            if (RBDotyczy.SelectedValue.ToString() == "1")
            {
                if (C.Length > 2 && N.Length > 2 && M.Length > 2 && T.Length > 2)
                {
                    ...
                    OleDbCommand cm = new OleDbCommand("INSERT INTO failure (what, topic, content, surname, email, floor, area, stand, sendDate) values ('OPROGRAMOWANIE', '" + T + "', '" + C + "', '" + N + "', '" + M + "', '" + F + "','" + A + "', '" + S + "', '" + date + "')", cnn);
